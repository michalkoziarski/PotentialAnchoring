...
def evaluate_trial(resampler_name, fold, level):
    RESULTS_PATH = Path(__file__).parents[0] / 'results_noise_level'
    RANDOM_STATE = 42
    ...
    for dataset_name in datasets.names():
        ...
        trial_name = f'{dataset_name}_{fold}_{resampler_name}_{level}'
        ...
        dataset = datasets.load(dataset_name, noise_level=level)
        ...
        for classifier_name in classifiers.keys():
            ...
            for scoring_function_name in scoring_functions.keys():
                ...
                row = [dataset_name, fold, classifier_name, resampler_name, level, scoring_function_name, score]
                rows.append(row)

        columns = ['Dataset', 'Fold', 'Classifier', 'Resampler', 'Level', 'Metric', 'Score']
...
if __name__ == '__main__':
    ...
    evaluate_trial(args.resampler_name, args.fold, args.level)
